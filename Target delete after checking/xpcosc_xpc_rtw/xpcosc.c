...
void xpcosc_output(int_T tid)
{
...

  {
    ...
    tmp = xpcosc_B.Integrator1;
    tmp_0 = xpcosc_B.SignalGenerator;
    xpcosc_Y.Outport[0] = tmp;
    xpcosc_Y.Outport[1] = tmp_0;
  }
...
void xpcosc_derivatives(void)
{
  /* Integrator Block: '<Root>/Integrator1' */
  {
    ((StateDerivatives_xpcosc *) xpcosc_rtM->ModelData.derivs)
      ->Integrator1_CSTATE = xpcosc_B.Integrator;
  }

  /* Integrator Block: '<Root>/Integrator' */
  {
    ((StateDerivatives_xpcosc *) xpcosc_rtM->ModelData.derivs)
      ->Integrator_CSTATE = xpcosc_B.Sum;
  }
}
...
void xpcosc_initialize(boolean_T firstTime)
{
  ...
  /* external mode info */
  xpcosc_rtM->Sizes.checksums[0] = (1000186991U);
  xpcosc_rtM->Sizes.checksums[1] = (688308565U);
  xpcosc_rtM->Sizes.checksums[2] = (3628972191U);
  xpcosc_rtM->Sizes.checksums[3] = (4088993745U);
