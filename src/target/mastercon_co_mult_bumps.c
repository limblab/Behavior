 ...
 */
#define S_FUNCTION_NAME mastercon_co_mult_bumps
...
static int movement_bumps = 0;
#define param_movement_bumps mxGetScalar(ssGetSFcnParam(S,18))

static int center_hold_bumps = 0;
#define param_center_hold_bumps mxGetScalar(ssGetSFcnParam(S,19))



static real_T first_target_shift;
#define param_first_target_shift mxGetScalar(ssGetSFcnParam(S,20))

static real_T target_angle_range;
#define param_target_angle_range mxGetScalar(ssGetSFcnParam(S,21))


static real_T bump_delay;
static real_T bump_delay_low = 0;
#define param_bump_delay_low mxGetScalar(ssGetSFcnParam(S, 22))

static real_T bump_delay_high = 0;
#define param_bump_delay_high mxGetScalar(ssGetSFcnParam(S, 23))
...
static void mdlCheckParameters(SimStruct *S)
{
	...
	movement_bumps = (int)param_movement_bumps;
	center_hold_bumps = (int)param_center_hold_bumps;
    
    bump_delay_low = param_bump_delay_low;
    bump_delay_high = param_bump_delay_high;
...
static void mdlUpdate(SimStruct *S, int_T tid) 
{
	...
	int *databurst_bump_params;
    real_T * databurst_bump_magnitude;
	byte *databurst_bump_type;
	...
	databurst_bump_params = (int *)(databurst_target_list + 4);
    databurst_bump_magnitude = (real_T *)(databurst_bump_params+2);
	databurst_bump_type = (byte *)(databurst_bump_magnitude + 1);
    databurst_center_hold = (real_T *)(databurst_bump_type+1);
    ...
    databurst_bump_delay = databurst_intertrial + 1;
    ...
    switch (state) {
        case STATE_PRETRIAL:
            ...
            if (bump_delay_high == bump_delay_low) {
                bump_delay = bump_delay_high;
            } else {
                bump_delay = bump_delay_low + (bump_delay_high - bump_delay_low)*((double)rand())/((double)RAND_MAX);
            }
            ...
            databurst_bump_params[0] = bump_duration;
			databurst_bump_params[1] = bump;
            databurst_bump_magnitude[0] = bump_magnitude;
			if (delay_bumps == 1) {
				databurst_bump_type[0] = 0x01; 
			}
			else if(center_hold_bumps ==1) {
				databurst_bump_type[0] = 0x02;
			}
			else if(movement_bumps == 1) {
				databurst_bump_type[0] = 0x03;
			}
			else {
				databurst_bump_type[0] = 0x00;
			}
            ...
            databurst_bump_delay[0] = bump_delay;
				/* reset counters */
    ...
    if (mode == MODE_BLOCK_CATCH) {
        ...
        if (bump_duration_counter > 0) {
            ...
            force_y = force_in[1] + sin(theta)*bump_magnitude;
                    ...
                    ( 
                      (state==STATE_MOVEMENT && sqrt(cursor[0]*cursor[0]+cursor[1]*cursor[1]) > (target_radius / 2)   && movement_bumps && elapsed_timer_time > bump_delay)|| 
                      (state==STATE_CENTER_DELAY && delay_bumps && elapsed_timer_time > bump_delay) ||
                      (state==STATE_CENTER_HOLD && center_hold_bumps && elapsed_timer_time > bump_delay) 
                    )
                  ) 
        {
