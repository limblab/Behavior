...
#define S_FUNCTION_NAME mastercon_3bc_arcTargets
...
void TwoBumpChoiceBehavior::doPreTrial(SimStruct *S) {
    ...
    targetBar->r  = params->target_radius;
	targetBar->theta  = 0; 
	targetBar->span    = 2*PI-0.00001;
	targetBar->height = params->target_size+.1;
         ...
         getState() == STATE_BUMP) 
	{
		...
		if (this->params->show_target_during_bump) {
			if (this->training_trial) {
				if(this->is_primary_target) {
                    outputs->targets[1] = (Target *)(this->targetBar);
                    outputs->targets[2] = (Target *)(this->primaryTarget);
			        outputs->targets[3] = nullTarget;
				} else {
		            outputs->targets[1] = (Target *)(this->targetBar);
                ...
                outputs->targets[1] = (Target *)(this->targetBar);
				outputs->targets[2] = (Target *)(this->primaryTarget);
		        outputs->targets[3] = (Target *)(this->secondaryTarget);
			}
		} else {
            outputs->targets[1] = nullTarget;
            outputs->targets[2] = nullTarget;
		...
		outputs->targets[0] = (Target *)(this->targetBar);
