 ...
 * bytes 35-38:	float		=> bump incriment
...
ForcedChoiceBehavior::ForcedChoiceBehavior(SimStruct *S) : RobotBehavior() {
    ...
    this->bindParamId(&params->bump_incr,           	11);
    ...
    this->bump_stair->setStepSize(
    this->bump_stair->setCurrentValue(
    this->stim_stair->setStepSize(1
    this->stim_stair->setCurrentValue(
}
...
void ForcedChoiceBehavior::doPreTrial(SimStruct *S) {
	...
	//set up the bump
    if(this->random->getDouble()>params->bump_prob){
        this->bump->peak_magnitude=this->bumpStair->getCurrentValue() *params->bump_incr + params->bump_floor;
    } else {
