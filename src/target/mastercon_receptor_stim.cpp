...
#define S_FUNCTION_NAME mastercon_receptor_stim
...
#define STATE_STIMULATION 3
#define STATE_MOVEMENT 4
#define STATE_TARGET_HOLD 5
#define STATE_TARGET_DELAY 6
/* 
 ...
 */
#define MY_CLASS_NAME ReceptorStim
class ReceptorStim : public RobotBehavior {
public:
	// You must implement these three public methods
	ReceptorStim(SimStruct *S);
	void update(SimStruct *S);
	void calculateOutputs(SimStruct *S);	
...
ReceptorStim::ReceptorStim(SimStruct *S) : RobotBehavior() {
    int i;
	...
	this->bindParamId(&params->disable_abort,			16);
	this->bindParamId(&params->green_hold,				18);
	this->bindParamId(&params->cumulative_hold,			19);
    this->bindParamId(&params->gto_stim,                20);
    this->bindParamId(&params->vibration_stim,          21);
...
void ReceptorStim::doPreTrial(SimStruct *S) {
	int i, j;
	double r, th;
	SquareTarget lastTarget = *targets[(int)params->num_targets];
	SquareTarget tmpTarget;
    
    if ((bool)params->gto_stim & !(bool)params->vibration_stim) {
        this->stimNum = random->getInteger(1, 25);
    } else if ((bool)params->vibration_stim & !(bool)params->gto_stim){
        this->stimNum = random->getInteger(26, 34);
        
    } else if ((bool)params->vibration_stim & (bool)params->gto_stim) {
        this->stimNum = random->getInteger(1, 34);
    }
    else {
        this->stimNum = 35;
    }
...
void ReceptorStim::update(SimStruct *S) {
    /* declarations */
	...
	switch (this->getState()) {
        ...
        case STATE_CENTER_HOLD:
			if (params->cumulative_hold && cumulativeHoldTimer->elapsedTime(S) > 6) {
			...
			} else if (stateTimer->elapsedTime(S) > 6) {
				/* next state depends on whether there are more targets */
				if (target_index == params->num_targets - 1) {
					/* no more targets */
					playTone(TONE_REWARD);
...
void ReceptorStim::calculateOutputs(SimStruct *S) {
    /* declarations */
	...
	if (db->isRunning()) {
		...
		switch (getState()) {
			...
			case STATE_INITIAL_MOVEMENT:
				...
				break;
            case STATE_CENTER_HOLD:
                outputs->word = WORD_RECEPTOR_STIM(this->stimNum);
                break;
			case STATE_TARGET_HOLD:
