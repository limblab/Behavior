 ...
 * Master Control block for behavior: many-targets
 */

#define S_FUNCTION_NAME mastercon_mt
...
static real_T multiple_targets = 0.0;  %snn
#define param_multiple_targets mxGetScalar(ssGetSFcnParam(S,11))  %snn
...
static void mdlCheckParameters(SimStruct *S)
{
  ...
  multiple_targets = param_multiple_targets;  %snn
...
%snn
				...
				if (target_index == (int)num_targets-1 || multiple_targets == 0) {
					...
					state_changed();
					reset_timer();
%snn replaced this section with another:
    ...
    if (state == STATE_DATA_BLOCK) {
        ...
        switch (state) {
            ...
            case STATE_MOVEMENT:
                ...
                break;             
            case STATE_TARGET_HOLD
                word = WORD_OUTER_TARGET_HOLD       /* added by snn */
                break;
            case = STATE_CONTINUE_MOVEMENT
                word =
                break;
