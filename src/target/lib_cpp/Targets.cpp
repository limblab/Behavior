...
// Ring gradient target
#define TARGET_TYPE_RING_GRADIENT 48
...
/**
 * First part of a ring gradient (must be followed by RingGradientB).
 * Represents a ring with graded color spectrum defined by a von Mises distribution
 * 
 */
class RingGradientA : public Target {
public:
	RingGradientA();
	RingGradientA(double radius, double width, int ringcol, double vm_m);
	void copyToOutputs(real_T *u, int offset);
...
/**
 * Creates a RingGradientA with the requested radius, width and colors.
 * @param radius the radius of the ring.
 * @param width the width of the ring.
 * @param ringcol the ring color.
 * @param colorB the secondary color.
 */
RingGradientA::RingGradientA(double radius, double width, int ringcol, double vm_m) {
	this->radius = radius;
	this->width = width;
 ...
 * @param offset the location to begin writing.
 */
void RingGradientA::copyToOutputs(real_T *u, int offset) {
	u[0+offset] = (real_T)(TARGET_TYPE_RING_GRADIENT);
	u[1+offset] = (real_T)(radius);
	u[2+offset] = (real_T)(width);
	u[3+offset] = (real_T)(ringcol);
	u[4+offset] = (real_T)(vm_m);
}
...
/**
 * Second part of a ring gradient target (must be preceded by RingGradientA).
 */
class RingGradientB : public Target {
public:
	RingGradientB();
	RingGradientB(double vm_k, double min_p, double max_p);
	void copyToOutputs(real_T *u, int offset);

	/** The kappa of the distribution */
	double vm_k;

	/** The min of all pdfs used (for scaling) */
	double min_p;

	/** The max of all pdfs used (for scaling) */
    double max_p;
...
/**
 * Creates a RingGradientB with the requested parameters.
 * @param vm_k kappa of the distribution
 * @param min_p minimum of all pdfs
 * @param max_p maximum of all pdfs
 */
RingGradientB::RingGradientB(double vm_k, double min_p, double max_p) {
	this->vm_k = vm_k;
	this->min_p = min_p;
	this->max_p = max_p;
}
 ...
 * @param offset the location to begin writing.
 */
void RingGradientB::copyToOutputs(real_T *u, int offset) {
	u[0+offset] = (real_T)(vm_k);
	u[1+offset] = (real_T)(min_p);
	u[2+offset] = (real_T)(max_p);
}
